...
class HydrawiseClient {
  ...
  async _liveStatusFor(zone) {
    const map = await this._liveRelayMap();
    if (!map) return null;
    const relay = map.get(String(this.relayId(zone)));
    ...
    if (t === 1) {
      ...
      return { state: 'running', detail: relay.name ? `${relay.name} running` : 'running', live: true };
    }
