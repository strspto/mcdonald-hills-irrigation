...
class HydrawiseClient {
  ...
  async _liveRelayMap() {
    ...
    this._liveMapPromise = (async () => {
      try {
        ...
        const map = new Map();
        for (const r of data.relays) map.set(String(r.relay_id), r);
        ...
        // now" source. Fold it into a map keyed by relay_id so we can
        // also pull time_left for a better "ends in" estimate.
        const runningMap = new Map();
        if (Array.isArray(data.running)) {
          for (const r of data.running) runningMap.set(String(r.relay_id), r);
        }

        return { map, runningMap };
      } catch (e) {
  ...
  async _liveStatusFor(zone) {
    ...
    if (!live) return null;
    const relayIdStr = String(this.relayId(zone));
    const relay = live.map.get(relayIdStr);
    if (!relay) return null; // relay_id doesn't match anything on the real controller
    ...
    if (t === 1 || live.runningMap.has(relayIdStr)) {
      ...
      const runningEntry = live.runningMap.get(relayIdStr);
