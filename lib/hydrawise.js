...
class HydrawiseClient {
   ...
   * "Hydrawise Relay ID" on the Zones page wins if present; otherwise
   * this looks up the live relay list by physical zone number (the same
   * matching used for status) to find the true relay_id. Falls back to
   * the plain zone number only if no live match is found — which is
   * fine for mock mode (no real API call happens) but WILL fail against
   * the real API, so this should only happen if Hydrawise is unreachable.
   */
  async relayId(zone) {
    ...
    if (!this.mock && this.apiKey) {
      ...
      if (relay) return relay.relay_id;
    }
