...
class HydrawiseClient {
  ...
  async runZone(zone, minutes, triggeredBy, programId = null) {
    if (zone && zone.control_type === 'lora') {
      return this._lora().runZone(zone, minutes, triggeredBy, programId);
    }
  ...
  async stopZone(zone, triggeredBy, programId = null) {
    if (zone && zone.control_type === 'lora') {
      return this._lora().stopZone(zone, triggeredBy, programId);
    }
    const relayId = await this.relayId(zone);
    if (this.mock) {
  ...
  // Lazily constructed so requiring ./lora doesn't create a
  // require-cycle risk if lora.js ever needs anything from here.
  _lora() {
    if (!this._loraClient) {
      const { LoraClient } = require('./lora');
      this._loraClient = new LoraClient();
    }
    return this._loraClient;
  }
  ...
  async zoneStatus(zoneOrId) {
    ...
    if (zone && zone.control_type === 'lora') {
      // No live Hydrawise-style status check for LoRa zones — a node
      // could add real status reporting later (see MSG_STATUS in
      // PROTOCOL.md), but for now this mirrors mock mode: derive
      // "is it running" from what we last told it to do, in run_log.
  ...
  async _liveStatusFor(zone) {
    ...
    // Our own run_log is the most trustworthy source for anything WE
    // just told the controller to do — the live relay snapshot only
    // refreshes at most once per its TTL (up to 60s, driven by
    // Hydrawise's own `nextpoll`), so a command issued after the last
    // refresh won't be reflected in `live` yet even though it already
    // succeeded. If our own record of the last action for this zone is
    // newer than the cached snapshot, trust it over the stale cache
    // rather than showing a result that contradicts the confirmation
    // the person just saw on-screen.
    const { rows: recentRows } = await query(
      `SELECT ts FROM run_log WHERE zone_id = $1 AND status = 'success'
       AND action IN ('run','runall','stop','stopall','suspend','resume')
       ORDER BY ts DESC LIMIT 1`,
      [zone.id]
    );
    const lastActionTs = recentRows[0] ? new Date(recentRows[0].ts).getTime() : 0;
    if (lastActionTs > _globalLiveCache.fetchedAt) {
      return this._localStatus(zone.id);
    }
    ...
    if (t === 1) {
      ...
      const endsAt = secsLeft ? Date.now() + secsLeft * 1000 : null;
      return {
        state: 'running',
