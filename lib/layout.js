...
async function header(req, { title, activeNav = '' } = {}) {
  ...
  <nav class="nav">
    ...
    <a href="/hotlist" class="${activeNav === 'hotlist' ? 'active' : ''}">Hot List</a>
...
(function() {
  ...
  // first click). Disabling a button DURING the very submit event it
  // triggered can, on some mobile browsers, cancel that same submission
  // outright — the click silently goes nowhere, no request, no page
  // reload, no log entry at all. Deferring the disable by one tick
  // (setTimeout 0) lets the browser actually start the navigation first;
  // the button still becomes unclickable fast enough to block a real
  // double-tap, just not so fast it eats the first one.
