...
app.post('/programs', requireAdmin, requireCsrf, async (req, res) => {
  ...
  if (action === 'save') {
    ...
    const startTime = req.body.start_time || '06:00';
    ...
    try {
      ...
      if (id) {
        await client.query(
          `UPDATE programs SET name=$1, start_time=$2, days_mask=$3, enabled=$4, seasonal_adjust_pct=$5, updated_at=now() WHERE id=$6`,
          [name, startTime, mask, enabled, seasonal, id]
        );
        await client.query('DELETE FROM program_zones WHERE program_id = $1', [id]);
      } else {
        const { rows } = await client.query(
          `INSERT INTO programs (name, start_time, days_mask, enabled, seasonal_adjust_pct, created_by) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id`,
          [name, startTime, mask, enabled, seasonal, u.id]
        );
        programId = rows[0].id;
      }
      let order = 0;
...
async function programsPage(req) {
  ...
  if (isAdmin && (editId || isNew)) {
    if (editId) {
      ...
      editing = { id: 0, name: '', start_time: '06:00', days_mask: 0, enabled: true, seasonal_adjust_pct: 100 };
    }
  }
  ...
  if (isAdmin && editing) {
    ...
    <div class="card">
      ...
      <form method="post" action="/programs">
        ...
        <div class="form-row"><label>Start time</label><input type="time" name="start_time" value="${e(editing.start_time)}" required></div>
  ...
  for (const p of programs) {
    ...
    <tr>
      <td>${e(p.name)}</td>
      <td>${e(fmtTime(DateTime.fromFormat(p.start_time, 'HH:mm').toMillis()))}</td>
      <td class="muted">${e(daysMaskToLabels(p.days_mask))}</td>
  ...
  html += `
<div class="page-title">
  <div><h1>Watering Programs</h1><p>Recurring schedules. The scheduler checks these every minute and starts zones automatically at their start time.</p></div>
...
<div class="card">
  ${!programs.length
    ...
    : `<table><thead><tr><th>Name</th><th>Start</th><th>Days</th><th>Zones</th><th>Status</th>${isAdmin ? '<th></th>' : ''}</tr></thead><tbody>${listRows}</tbody></table>`}
</div>` + footer();
  return html;
}
...
async function schedulerTick() {
  try {
    ...
    const { rows: programs } = await query('SELECT * FROM programs WHERE enabled = true');
    const hc = await HydrawiseClient.create();
    ...
    for (const p of programs) {
      ...
      for (const z of zones) {
        const minutes = Math.max(1, Math.round(z.duration_minutes * (p.seasonal_adjust_pct / 100)));
        await hc.runZone(z, minutes, 'scheduler', p.id);
      }
      console.log(`[scheduler] Ran program "${p.name}" (${zones.length} zones)`);
    }
  } catch (err) {
